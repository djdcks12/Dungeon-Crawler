...
필요한 작업:
🔧 층별 시간 제한 시스템 (1층 10분 → 10층 총 60분)
🔧 시간 carryover 시스템 (빠른 클리어 시 다음 층에 시간 추가)
🔧 강제 방출 시스템 (시간 초과 시 자동 퇴장)
🔧 유연한 맵 구조 시스템 (맵툴 연동 가능)
🔧 포탈 시스템 (하위층 포탈 닫힘)
```

#### 2. UI/UX 완성도 향상  
**진행도: 70%** (기본 UI 완료, 폴리싱 필요)
```
완료됨:
...
필요한 작업:
🔧 StatsUI 폴리싱 - 종족별/영혼별 스탯 표시 개선
🔧 SkillUI 개선 - 종족별 스킬 트리 시각화  
🔧 DungeonUI - 층별 진행도, 시간 표시, 미니맵
🔧 DeathUI - 영혼 선택 인터페이스
🔧 메인 HUD - HP/MP바, 경험치바, 골드 표시
```

### 📈 우선순위 2: 핵심 확장

#### 3. PvP 밸런싱 시스템
**진행도: 40%** (기본 PvP 작동, 밸런스 조정 필요)
```
완료됨:
✅ 기본 PvP 가능 (던전 내에서)
✅ 파티원 간 공격 방지
...
#### 4. Economy System 확장
**진행도: 50%** (기본 골드 시스템 있음, 경제 구조 확장 필요)
```
완료됨:  
✅ 골드 기반 스킬 구매
✅ 몬스터 처치 시 골드 드롭
...
필요한 작업:
🔧 층별 골드 드롭량 밸런싱
🔧 귀환 비용 시스템 (첫 귀환 100골드, 이후 1.5배씩 증가)
🔧 NPC 상점 시스템 (기본 장비, 소모품)
🔧 수리 시스템 (장비 내구도)
```
